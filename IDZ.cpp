// ============================================================================
// МОДЕЛИРОВАНИЕ ТРАЕКТОРИИ ПОЛЕТА САМОЛЕТА ИЛ-76
// ============================================================================
#include <iostream>
#include <vector>
#include <cmath>
...
const double BASELINE_MASS = 155000.0;   // Стартовая масса, кг
const double WING_AREA = 300.0;     // Площадь крыла, м²

// Двигательная установка
const double ENGINE_THRUST = 4 * 120000.0; // Суммарная тяга, Н
...
const double TARGET_SPEED_KPH = 800.0;
...
public:
    ...
    Aircraft(double mass = BASELINE_MASS, double area = WING_AREA)
        ...
        induced_drag_coeff = 0.42;             // K
        max_lift_coefficient = 1.42;            // Cy_max
    }
...
public:
    ...
    FlightTrajectory calculateFlightPath(Aircraft& aircraft, double max_flight_time = 600.0) {
        ...
        std::cout << "МОДЕЛИРОВАНИЕ ПОЛЕТА IL76" << std::endl;
...
int main() {
    ...
    try {
        std::cout << "\n" << std::string(60, '=') << std::endl;
        std::cout << " СИМУЛЯТОР ПОЛЕТА IL76" << std::endl;
        ...
        FlightTrajectory optimal_path = flight_planner.calculateFlightPath(il76_model, 300.0);

        // Сохранение результатов
        optimal_path.exportToCSV("IL76_flight_profile.csv");
